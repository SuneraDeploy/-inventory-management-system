...
class salesrep_fanta extends StatelessWidget {
  final String salesRepId;
  final String title;

  const salesrep_fanta({super.key, required this.salesRepId, required this.title});
        ...
        Container(
          ...
          decoration: const BoxDecoration(
            image: DecorationImage(
              image: AssetImage("images/salesrep_view_fanta.png"),
              fit: BoxFit.fill,
            ),
          ),
        ),
...
class DataDisplayTableWithPrefix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: fetchDocumentsWithPrefix(salesRepId, "FN"),
...
class DataDisplayTableWithTitle extends StatelessWidget {
  ...
  Future<List<Map<String, dynamic>>> fetchDocuments(String title) async {
    ...
    try {
      final querySnapshot = await FirebaseFirestore.instance
          .collection(title)
          .where('request_product_id', whereIn: ['FN1', 'FN2', 'FN3', 'FN4', 'FN5']) // Filter for these IDs
          .get();
