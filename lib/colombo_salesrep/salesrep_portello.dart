...
class salesrep_portello extends StatelessWidget {
  final String salesRepId;
  final String title;

  const salesrep_portello({super.key, required this.salesRepId, required this.title});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Portello Stocks'),
        ...
        Container(
          ...
          decoration: const BoxDecoration(
            image: DecorationImage(
              image: AssetImage("images/salesrep_view_portello.png"),
              fit: BoxFit.fill,
            ),
          ),
        ),
...
class DataDisplayTableWithPrefix extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<Map<String, dynamic>>>(
      future: fetchDocumentsWithPrefix(salesRepId, "PO"),
...
class DataDisplayTableWithTitle extends StatelessWidget {
  ...
  Future<List<Map<String, dynamic>>> fetchDocuments(String title) async {
    ...
    try {
      final querySnapshot = await FirebaseFirestore.instance
          .collection(title)
          .where('request_product_id', whereIn: ['PO1', 'PO2', 'PO3', 'PO4', 'PO5']) // Filter for these IDs
          .get();
