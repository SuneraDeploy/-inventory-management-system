...
import 'salesrep_stock_view.dart';
import 'salesrep_yourstock_view.dart';



class salesrep_stock_editing extends StatelessWidget {
  final String salesRepId;
  const salesrep_stock_editing({super.key, required this.salesRepId});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
        appBar: AppBar(
          title: const Text('Sales Rep Stock Editing'),
...
class _BackgroundImageState extends State<BackgroundImage> {
  ...
  void _addDataToFirestore() async {
    ...
    try {
      ...
      final collectionRef = FirebaseFirestore.instance.collection(widget.salesRepId);
    ...
    void _addRequestProductToFirestore() async {
    ...
    if (requestProductId.isEmpty ||
        requestProductName.isEmpty ||
        requestQuantity.isEmpty) {
      // Show error if fields are empty
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('All fields are required!')),
      );
      return;
        ...
        try {
      // Reference Firestore collection for request products
      final requestCollectionRef = FirebaseFirestore.instance
          .collection('${widget.salesRepId} request products');

      // Add a new document to the collection
      await requestCollectionRef.add({
      ...
      // Clear request fields and show success message
      ...
      _requestQuantityController.clear();
        ...
        SnackBar(
          content: Text(
              'Request for $requestProductName added successfully!'),
        ),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        child: Stack(
          children: [
              ...
              child: SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => salesrep_yourstock_view(salesRepId: widget.salesRepId, title: '',)),
                    );
                  },
            ...
            Positioned(
              ...
              left: MediaQuery.of(context).size.width * 0.5 - buttonWidth / 2,
              child: SizedBox(
                width: buttonWidth,
                height: buttonHeight,
                child: ElevatedButton(
                  style: ButtonStyle(
                   ...
                   onPressed: _addRequestProductToFirestore,
